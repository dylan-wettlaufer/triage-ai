    ...
    async def upload_files(self, files: List[UploadFile], triage_id: str) -> List[Tuple[str, str]]:
        ...
        for file in files: # Iterate over each file
            ...
            try:
                ...
                # upload the file
                response = self.supabase.storage.from_(self.storage_bucket).upload(path=file_path, file=contents, 
                                                                                   file_options={"content-type": file.content_type if file.content_type else "application/octet-stream","upsert": True
                })

                if response.data and 'path' in response.data: # Check if the upload was successful
